...
int main() {
    try {
        ...
        constexpr int N = 4;
        std::vector<zombie::WalkState<Real, DIM>> hostStates(N);

        for (int i = 0; i < N; ++i) {
            hostStates[i].currentPt[0] = float(i);
            hostStates[i].currentPt[1] = float(i + 10);
        ...
        WalkStateLite liteStates[N];
        for (int i = 0; i < N; ++i) {
            liteStates[i] = packState(hostStates[i]);
        }

        {
            sycl::buffer<WalkStateLite, 1> buf(liteStates, sycl::range<1>(N));
            q.submit([&](sycl::handler& h) {
                auto acc = buf.get_access<sycl::access::mode::read_write>(h);
                h.parallel_for(sycl::range<1>(N), [=](sycl::id<1> idx) {
                    int i = idx[0];
                    acc[i].currentPt[0] += 0.5f;
                    acc[i].currentPt[1] += 1.0f;
                    acc[i].throughput *= 0.8f;
                    acc[i].walkLength += 1;
                });
            });
            q.wait();
        ...
        for (int i = 0; i < N; ++i) {
            unpackState(liteStates[i], hostStates[i]);
        }
